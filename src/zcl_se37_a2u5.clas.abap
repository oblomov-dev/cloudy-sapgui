...
CLASS zcl_se37_a2u5 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    DATA(page) = view->open( n = `View` ns = `mvc`
            ...
            )->a( n = `title`          v = `Function Builder: Initial Screen`
    ...
    DATA(tab) = page->open( `Table`
        )->a( n = `items`   v = client->_bind( mt_functions )
        )->a( n = `sticky`  v = `ColumnHeaders`
        )->a( n = `growing` v = `true`
        )->a( n = `class`   v = `sapUiSizeCompact` ).

    DATA(cols) = tab->open( `columns` ).
    cols->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Function Module` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Function Group` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `RFC` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Short Text` )->shut( ).
    ...
    cols->shut( )->open( `items`
                ...
                )->leaf( `Text` )->a( n = `text` v = `{FUNCNAME}`
                )->leaf( `Text` )->a( n = `text` v = `{AREA}`
                )->leaf( `Text` )->a( n = `text` v = `{RFC}`
                )->leaf( `Text` )->a( n = `text` v = `{STEXT}` ).
  ...
  METHOD view_detail.
    ...
    DATA(page) = view->open( n = `View` ns = `mvc`
            ...
            )->a( n = `title`          v = |Function Builder: Display { mv_current }|
    ...
    DATA(tab) = page->open( `Table`
        )->a( n = `items`   v = client->_bind( mt_params )
        )->a( n = `sticky`  v = `ColumnHeaders`
        )->a( n = `growing` v = `true`
        )->a( n = `class`   v = `sapUiSizeCompact` ).

    DATA(cols) = tab->open( `columns` ).
    cols->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Kind` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Parameter` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Typing` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Associated Type` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Optional` )->shut(
        )->open( `Column` )->leaf( `Text` )->a( n = `text` v = `Default Value` )->shut( ).

    cols->shut( )->open( `items`
        )->open( `ColumnListItem` )->open( `cells`
            )->leaf( `Text` )->a( n = `text` v = `{KIND}`
            )->leaf( `Text` )->a( n = `text` v = `{PARAMETER}`
            )->leaf( `Text` )->a( n = `text` v = `{TYPING}`
            )->leaf( `Text` )->a( n = `text` v = `{REFERENCE}`
            )->leaf( `Text` )->a( n = `text` v = `{OPTIONAL}`
            )->leaf( `Text` )->a( n = `text` v = `{DEFAULT}` ).
    ...
    page->open( `footer` )->open( `OverflowToolbar`
        ...
        )->leaf( `Label` )->a( n = `text` v = |{ lines( mt_params ) } parameter(s)| ).
